...
<script type="text/javascript">
	...
	function approvalNameList(data){
		...
		$.each(data.workLog,function(idx,item){
			...
			.append($('<td class="apv'+idx+'">').html('<div onclick="approval(this'+','+ thisdate+','+ id +')" id="approval" style="cursor:pointer;">'+apv+'</div>'))
			.appendTo($('#tbody1'));
	...
	function approval(aa,date,id){
		alert("date , id : "+date + id);
		alert(aa.text);
		$.ajax({
		    ...
		    success: function(data) {
				...
				if(approval==="N"){
					$("#approval").text("결재");
					$("#approval").css("color", "red");
					
				}else{
					$("#approval").text("완료");
					$("#approval").css("color", "blue");
				}
		    },
		    error:function(xhr, status, message) {
		        alert(" status: "+status+" er:"+message);
	...
	<div id="subContainer">
				...
				<c:choose>
					...
					<c:otherwise>
						<tr>
							...
							<td>
								<a  style="color: ${workLogOne.confirm3Ok eq 'N'?'red':'blue'}" id="approval" 
		...
		<div id="floatRight">
				<table>
					...
					<col width="60px"/>
					<tr id="tr">
						<td id="td1">선택일</td><td>${workLogOne.thisdate}</td><td  id="td1">작성자</td><td>${workLogOne.userid }</td><td  id="td1">출근시간</td><td>${workLogOne.workstarttime }</td><td  id="td1">퇴근시간</td><td>${workLogOne.workendtime }</td>
						<td style="background-color: white; border: 1px solid white; border-right: #dcdcdc "></td>
						<td><a class="button1" onmouseover="this.className='Button_Over1';" onmouseout="this.className='button1';">추가</a></td>
					</tr>
				</table>
